                            ...
                            {projects.map((project, idx) => (
                                ...
                                >
                                    <motion.div
                                        ...
                                        className="group/card relative min-h-[500px] flex flex-col rounded-2xl overflow-hidden bg-[var(--bg-secondary)] border border-[var(--card-border)] hover-border-accent transition-all duration-500 shadow-sm cursor-pointer"
                                    >
                                        {/* Decorative Glow */}
                ...
                {selectedProject && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                        ...
                        <motion.div 
                            ...
                            className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-[var(--bg-primary)] border border-[var(--card-border)] md:border-emerald-500/30 rounded-2xl shadow-2xl z-10 custom-scrollbar flex flex-col md:flex-row"
                        >
                            <button 
                                onClick={() => setSelectedProject(null)}
                                className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-black/50 text-white hover:bg-emerald-500 hover:text-black transition-colors z-20"
                            >
                                <i className="fas fa-times"></i>
                            </button>

                            {/* Modal Image */}
                            <div className="w-full md:w-2/5 h-64 md:h-auto overflow-hidden relative bg-black/20">
                                {selectedProject.image ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center">
                                        <i className={`fas ${selectedProject.icon || 'fa-code'} text-7xl text-emerald-400 opacity-20`}></i>
                                    </div>
                                )}
                                <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg-primary)] md:from-transparent md:bg-gradient-to-r via-transparent to-transparent"></div>
                            </div>

                            {/* Modal Content */}
                            <div className="w-full md:w-3/5 p-6 md:p-8 flex flex-col bg-[var(--bg-primary)]">
                                <h3 className="text-2xl md:text-3xl font-bold text-primary mb-4 pr-8 text-emerald-400">
                                    {selectedProject.title}
                                </h3>
                                
                                <div className="prose prose-invert max-w-none text-secondary mb-6 leading-relaxed">
                                ...
                                <div className="mb-8">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {selectedProject.tech.map(t => (
                                            <span key={t} className="px-3 py-1.5 bg-emerald-500/10 text-emerald-400 text-xs font-bold tracking-wide rounded-lg border border-emerald-500/20">
                                                {t}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="mt-auto flex gap-4">
                                    {selectedProject.live && (
                                        <a 
                                            ...
                                            target="_blank" 
                                            rel="noreferrer"
                                            className="flex-1 py-3 bg-gradient-to-r from-emerald-600 to-emerald-400 hover:from-emerald-500 hover:to-emerald-300 text-white text-center rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(52,211,153,0.3)] hover:shadow-[0_0_25px_rgba(52,211,153,0.5)]"
                                        >
                                    ...
                                    {selectedProject.github && (
                                        <a 
                                            ...
                                            target="_blank" 
                                            rel="noreferrer"
                                            className="flex-1 py-3 bg-transparent border border-gray-600 hover:border-emerald-500 hover:text-emerald-400 text-primary text-center rounded-xl font-bold transition-all flex items-center justify-center gap-2"
                                        >
                                            <i className="fab fa-github text-lg"></i> View Code
                                        </a>
                                    )}
                                </div>
                            </div>
